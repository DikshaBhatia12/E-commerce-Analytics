...
rfm_segments AS (
    SELECT
        ...
        CASE
            -- Order matters here: more specific/narrower conditions must be
            -- checked before broader ones, or the broader rule wins first
            -- and the narrower segment can never be reached. (The original
            -- version of this CASE checked "At-risk" before "Cannot lose
            -- them" and had no rule that could ever fall through to "Needs
            -- attention" — both segments came out permanently empty.)
            ...
            WHEN r_score_fixed <= 2 AND f_score = 3
                THEN 'At-risk'
            WHEN (r_score_fixed >= 4 AND f_score = 3)
              OR (r_score_fixed = 3 AND f_score >= 4)
            ...
            WHEN r_score_fixed <= 2 AND f_score <= 2
                THEN 'Lost'
            ELSE 'Needs attention'   -- r=3 AND f=3: average on both axes
        END                                         AS segment
    FROM rfm_scores
)
